...
int OPENSSL_init_crypto(uint64_t opts, const OPENSSL_INIT_SETTINGS *settings)
{
    ...
    for(int i = 0; i <= 8; ++i){
        eapp_print("sp(%d): %lx", i * 8, *((unsigned long*)sp_ptr + i));
    }
    if (stopped) {
     ...
     */
    eapp_print("Before ossl_init_base()\n");
     ...
     * return above.
     */
    eapp_print("Before ossl_init_no_register_atexit()\n");
    ...
    eapp_print("Before ossl_init_load_crypto_nodelete()\n");
    if (!RUN_ONCE(&load_crypto_nodelete, ossl_init_load_crypto_nodelete))
        return 0;
    eapp_print("Before OPENSSL_INIT_NO_LOAD_CRYPTO_STRINGS()\n");
    if ((opts & OPENSSL_INIT_NO_LOAD_CRYPTO_STRINGS)
            && !RUN_ONCE_ALT(&load_crypto_strings,
                             ossl_init_no_load_crypto_strings,
                             ossl_init_load_crypto_strings))
        return 0;
    eapp_print("Before OPENSSL_INIT_LOAD_CRYPTO_STRINGS()\n");
    if ((opts & OPENSSL_INIT_LOAD_CRYPTO_STRINGS)
            && !RUN_ONCE(&load_crypto_strings, ossl_init_load_crypto_strings))
        return 0;
    eapp_print("Before OPENSSL_INIT_NO_ADD_ALL_CIPHERS()\n");
    if ((opts & OPENSSL_INIT_NO_ADD_ALL_CIPHERS)
            && !RUN_ONCE_ALT(&add_all_ciphers, ossl_init_no_add_all_ciphers,
                             ossl_init_add_all_ciphers))
        return 0;
    eapp_print("Before OPENSSL_INIT_ADD_ALL_CIPHERS()\n");
    if ((opts & OPENSSL_INIT_ADD_ALL_CIPHERS)
            && !RUN_ONCE(&add_all_ciphers, ossl_init_add_all_ciphers))
        return 0;
    eapp_print("Before OPENSSL_INIT_NO_ADD_ALL_DIGESTS()\n");
    if ((opts & OPENSSL_INIT_NO_ADD_ALL_DIGESTS)
            && !RUN_ONCE_ALT(&add_all_digests, ossl_init_no_add_all_digests,
                             ossl_init_add_all_digests))
        return 0;
    eapp_print("Before OPENSSL_INIT_ADD_ALL_DIGESTS()\n");
...
#endif
    eapp_print("Before 757()\n");
#ifndef OPENSSL_NO_COMP
    if ((opts & OPENSSL_INIT_ZLIB)
            && !RUN_ONCE(&zlib, ossl_init_zlib))
        return 0;
#endif
    eapp_print("Before 763()\n");
    for(int i = 0; i <= 8; ++i){
        eapp_print("sp(%d): %lx", i * 8, *((unsigned long*)sp_ptr + i));
    }
    return 1;
