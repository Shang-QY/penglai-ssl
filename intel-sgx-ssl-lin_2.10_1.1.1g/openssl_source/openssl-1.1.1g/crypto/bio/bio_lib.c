...
static int bio_read_intern(BIO *b, void *data, size_t dlen, size_t *readbytes)
{
    ...
    if ((b->callback != NULL || b->callback_ex != NULL) &&
        ((ret = (int)bio_call_callback(b, BIO_CB_READ, data, dlen, 0, 0L, 1L,
                                       NULL)) <= 0))
    {
        eapp_print("[bio_read_intern] Before 264 ret: %d\n", ret);
        return ret;
    }
    ...
    eapp_print("[bio_read_intern] After 274 ret: %d\n", ret);
    if (ret > 0)
    ...
    if (ret > 0 && *readbytes > dlen) {
        eapp_print("[bio_read_intern] After 284 ret: %d\n", ret);
...
int BIO_read(BIO *b, void *data, int dlen)
{
    ...
    eapp_print("[BIO_read] After bio_read_intern ret: %d\n", ret);
    if (ret > 0) {
...
int BIO_gets(BIO *b, char *buf, int size)
{
    int ret;
    size_t readbytes = 0;
    eapp_print("In BIO_gets\n");
    if ((b == NULL) || (b->method == NULL) || (b->method->bgets == NULL)) {
        BIOerr(BIO_F_BIO_GETS, BIO_R_UNSUPPORTED_METHOD);
        return -2;
    }
    eapp_print("Before if (size < 0)\n");
    if (size < 0) {
        BIOerr(BIO_F_BIO_GETS, BIO_R_INVALID_ARGUMENT);
        return 0;
    }
    eapp_print("Before if bio_call_callback\n");
    if (b->callback != NULL || b->callback_ex != NULL) {
        ret = (int)bio_call_callback(b, BIO_CB_GETS, buf, size, 0, 0L, 1, NULL);
        if (ret <= 0)
            return ret;
    }
    eapp_print("Before if (!b->init)\n");
    if (!b->init) {
        BIOerr(BIO_F_BIO_GETS, BIO_R_UNINITIALIZED);
        return -2;
    }
    eapp_print("Before bgets\n");
    ...
    eapp_print("Before (int)bio_call_callback, ret: %d, readbyte: %d\n", ret, readbytes);
    if (b->callback != NULL || b->callback_ex != NULL)
        ret = (int)bio_call_callback(b, BIO_CB_GETS | BIO_CB_RETURN, buf, size,
                                     0, 0L, ret, &readbytes);
    eapp_print("Before return, ret: %d, readbyte: %d\n", ret, readbytes);
    if (ret > 0) {
        /* Shouldn't happen */
