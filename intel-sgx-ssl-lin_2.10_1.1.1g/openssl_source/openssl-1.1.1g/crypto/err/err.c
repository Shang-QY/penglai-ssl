...
{
    unsigned long sp_ptr, ra;
    asm volatile("mv %0 ,sp" : "=r"(sp_ptr));
    asm volatile("mv %0 ,ra" : "=r"(ra));
    eapp_print("sp_ptr: %lx, ra: %lx", sp_ptr, ra);
    eapp_print("In do_err_strings_init, before OPENSSL_init_crypto");
    if (!OPENSSL_init_crypto(0, NULL))
        return 0;
    eapp_print("In do_err_strings_init, end OPENSSL_init_crypto");
...
int ERR_load_ERR_strings(void)
{
    ...
    asm volatile("mv %0 ,ra" : "=r"(ra));
    eapp_print("sp_ptr: %lx, ra: %lx", sp_ptr, ra);
     eapp_print("In ERR_load_ERR_strings");
#ifndef OPENSSL_NO_ERR
    if (!RUN_ONCE(&err_string_init, do_err_strings_init))
        return 0;

    eapp_print("In ERR_load_ERR_strings, After do_err_strings_init");
...
#endif
    eapp_print("End ERR_load_ERR_strings");
    return 1;
}
...
{
    fprintf(stdout, "In ERR_put_error()\n");
    eapp_print("In ERR_put_error()\n");
    ERR_STATE *es;
...
#endif
    eapp_print("Before ERR_get_state()\n");
    es = ERR_get_state();
    if (es == NULL)
        return;
    ...
    eapp_print("Before ERR_PACK()\n");
    es->err_buffer[es->top] = ERR_PACK(lib, func, reason);
    es->err_file[es->top] = file;
    es->err_line[es->top] = line;
    eapp_print("Before err_clear_data()\n");
...
{
    ERR_STATE *state;
    eapp_print("Before get_last_sys_error()\n");
    int saveerrno = get_last_sys_error();

    eapp_print("Before OPENSSL_init_crypto()\n");
    ...
    eapp_print("Before CRYPTO_THREAD_get_local()\n");
    state = CRYPTO_THREAD_get_local(&err_thread_local);
    if (state == (ERR_STATE*)-1)
        return NULL;

    if (state == NULL) {
        eapp_print("Before CRYPTO_THREAD_set_local()\n");
        ...
        eapp_print("Before ossl_init_thread_start()\n");
    ...
    eapp_print("Before set_sys_error()\n");
    set_sys_error(saveerrno);

    eapp_print("End ERR_get_state()\n");
    return state;
}
