                                    ...
                                    unsigned int flags)
{
    ...
    eapp_print("In pem_bytes_read_bio_flags()\n");
    do {
        eapp_print("In pem_free()\n");
        pem_free(nm, flags, 0);
    ...
    eapp_print("Before PEM_get_EVP_CIPHER_INFO()\n");
    if (!PEM_get_EVP_CIPHER_INFO(header, &cipher))
        goto err;
    eapp_print("Before PEM_do_header()\n");
...
static int get_name(BIO *bp, char **name, unsigned int flags)
{
    ...
    eapp_print("Before do\n");
    do {
        eapp_print("Before BIO_gets\n");
        len = BIO_gets(bp, linebuf, LINESIZE);
        eapp_print("After BIO_gets\n");
        if (len <= 0) {
            eapp_print("Before PEMerr\n");
                               ...
                               unsigned int flags)
{
    ...
    eapp_print("Before for (line = 0; ; line++)\n");
    for (line = 0; ; line++) {
        flags_mask = ~0u;
        len = BIO_gets(bp, linebuf, LINESIZE);
        if (len <= 0) {
        ...
        eapp_print("Before MAYBE_HEADER()\n");
                    ...
                    unsigned char **data, long *len_out, unsigned int flags)
{
    ...
    *data = NULL;
    ...
    eapp_print("Before BIO_s_secmem()\n");
    ...
    eapp_print("Before get_name()\n");
    if (!get_name(bp, &name, flags))
        goto end;
    eapp_print("Before get_header_and_data()\n");
    if (!get_header_and_data(bp, &headerB, &dataB, name, flags))
        goto end;
    eapp_print("Before EVP_DecodeInit()\n");
    EVP_DecodeInit(ctx);
