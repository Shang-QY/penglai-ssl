                ...
                size_t *readbytes)
{
    ...
    if (n == 0){
        eapp_print("[ssl3_read_n] Before 186\n");
        return 0;
    }
    ...
    if (rb->buf == NULL)
        if (!ssl3_setup_read_buffer(s)) {
            eapp_print("[ssl3_read_n] Before 191\n");
    ...
    if (SSL_IS_DTLS(s)) {
        if (left == 0 && extend){
            eapp_print("[ssl3_read_n] Before 251\n");
            return 0;
        }
        if (left > 0 && n > left)
            n = left;
    ...
    if (n > rb->len - rb->offset) {
        /* does not happen */
        eapp_print("[ssl3_read_n] Before 271\n");
    ...
    while (left < n) {
        eapp_print("[ssl3_read_n] Before 289\n");
        size_t bioread = 0;
        int ret;
        ...
        if (s->rbio != NULL) {
            ...
            eapp_print("[ssl3_read_n] Before 316 ret: %d\n", ret);
            if (ret >= 0){
                eapp_print("[ssl3_read_n] Before 305 ret: %d\n", ret);
                bioread = ret;
            }
        } else {
            eapp_print("[ssl3_read_n] Before 307\n");
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_SSL3_READ_N,
                    ...
                    size_t len, int peek, size_t *readbytes)
{
    ...
    if (!SSL3_BUFFER_is_initialised(rbuf)) {
        /* Not initialized yet */
        if (!ssl3_setup_read_buffer(s)) {
            eapp_print("[ssl3_read_bytes] Before 1232\n");
            /* SSLfatal() already called */
            return -1;
        }
    }
    ...
    if ((type && (type != SSL3_RT_APPLICATION_DATA)
         && (type != SSL3_RT_HANDSHAKE)) || (peek
                                             && (type !=
                                                 SSL3_RT_APPLICATION_DATA))) {
        eapp_print("[ssl3_read_bytes] Before 1242\n");
    ...
    if (!ossl_statem_get_in_handshake(s) && SSL_in_init(s)) {
        eapp_print("[ssl3_read_bytes] Before 1279\n");
        /* type == SSL3_RT_APPLICATION_DATA */
        i = s->handshake_func(s);
        /* SSLfatal() already called */
        if (i < 0){
            eapp_print("[ssl3_read_bytes] Before 1284\n");
            return i;
        }
        if (i == 0){
            eapp_print("[ssl3_read_bytes] Before 1288\n");
            return -1;
        }
    }
 start:
    ...
    do {
        ...
        if (num_recs == 0) {
            ...
            if (ret <= 0) {
                eapp_print("[ssl3_read_bytes] Before 1310\n");
                /* SSLfatal() already called if appropriate */
                return ret;
            }
            num_recs = RECORD_LAYER_get_numrpipes(&s->rlayer);
            if (num_recs == 0) {
                eapp_print("[ssl3_read_bytes] Before 1315\n");
                /* Shouldn't happen */
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_SSL3_READ_BYTES,
                         ERR_R_INTERNAL_ERROR);
                return -1;
    ...
    if (s->rlayer.handshake_fragment_len > 0
            ...
            && SSL_IS_TLS13(s)) {
        eapp_print("[ssl3_read_bytes] Before 1338\n");
    ...
    if (s->s3->change_cipher_spec /* set when we receive ChangeCipherSpec,
        ...
        && (SSL3_RECORD_get_type(rr) != SSL3_RT_HANDSHAKE)) {
        eapp_print("[ssl3_read_bytes] Before 1357\n");
        SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_READ_BYTES,
    ...
    if (s->shutdown & SSL_RECEIVED_SHUTDOWN) {
        ...
        s->rwstate = SSL_NOTHING;
        eapp_print("[ssl3_read_bytes] Before 1370\n");
        return 0;
    }
    ...
    if (type == SSL3_RECORD_get_type(rr)
        ...
        if (SSL_in_init(s) && (type == SSL3_RT_APPLICATION_DATA) &&
            (s->enc_read_ctx == NULL)) {
            eapp_print("[ssl3_read_bytes] Before 1389\n");
            SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_READ_BYTES,
        ...
        if (type == SSL3_RT_HANDSHAKE
            ...
            eapp_print("[ssl3_read_bytes] Before 1398\n");
            SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_READ_BYTES,
        ...
        if (len == 0) {
             ...
             */
            eapp_print("[ssl3_read_bytes] Before 1413\n");
    ...
    if (rr->rec_version == SSL2_VERSION) {
         ...
         */
        eapp_print("[ssl3_read_bytes] Before 1477\n");
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_SSL3_READ_BYTES,
                 ERR_R_INTERNAL_ERROR);
        return -1;
    }
    ...
    if (s->method->version == TLS_ANY_VERSION
        ...
        eapp_print("[ssl3_read_bytes] Before 1492\n");
        SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_READ_BYTES,
    ...
    if (SSL3_RECORD_get_type(rr) == SSL3_RT_ALERT) {
        ...
        if (!PACKET_buf_init(&alert, alert_bytes, SSL3_RECORD_get_length(rr))
            ...
            eapp_print("[ssl3_read_bytes] Before 1513\n");
            SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_READ_BYTES,
                     SSL_R_INVALID_ALERT);
            return -1;
        }
        ...
        if (alert_level == SSL3_AL_WARNING
            ...
            if (s->rlayer.alert_count == MAX_WARN_ALERT_COUNT) {
                eapp_print("[ssl3_read_bytes] Before 1540\n");
                SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_READ_BYTES,
        ...
        if (is_tls13 && alert_descr == SSL_AD_USER_CANCELLED) {
            ...
            eapp_print("[ssl3_read_bytes] Before 1556\n");
            return 0;
            ...
            eapp_print("[ssl3_read_bytes] Before 1570\n");
            return 0;
             ...
             */
            eapp_print("[ssl3_read_bytes] Before 1581\n");
        ...
        eapp_print("[ssl3_read_bytes] Before 1589\n");
    ...
    if ((s->shutdown & SSL_SENT_SHUTDOWN) != 0) {
        if (SSL3_RECORD_get_type(rr) == SSL3_RT_HANDSHAKE) {
            ...
            if (!SSL_IS_TLS13(s)) {
                ...
                eapp_print("[ssl3_read_bytes] Before 1617\n");
                return -1;
            }
        } else {
            /*
            ...
            SSL3_RECORD_set_length(rr, 0);
            SSL3_RECORD_set_read(rr);
            eapp_print("[ssl3_read_bytes] Before 1630\n");
            SSLfatal(s, SSL_AD_NO_ALERT, SSL_F_SSL3_READ_BYTES,
    ...
    if (SSL3_RECORD_get_type(rr) == SSL3_RT_CHANGE_CIPHER_SPEC) {
        eapp_print("[ssl3_read_bytes] Before 1666\n");
        SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_READ_BYTES,
                 SSL_R_CCS_RECEIVED_EARLY);
        return -1;
    }
    ...
    if ((s->rlayer.handshake_fragment_len >= 4)
        ...
        /* SSLfatal() already called if appropriate */
        if (i < 0){
            eapp_print("[ssl3_read_bytes] Before 1686\n");
            return i;
        }
        if (i == 0) {
            eapp_print("[ssl3_read_bytes] Before 1690\n");
            return -1;
        }
         ...
         */
        if (ined){
            eapp_print("[ssl3_read_bytes] Before 1700\n");
            return -1;
        }
        ...
        if (!(s->mode & SSL_MODE_AUTO_RETRY)) {
            if (SSL3_BUFFER_get_left(rbuf) == 0) {
                ...
                eapp_print("[ssl3_read_bytes] Before 1718\n");
                return -1;
            }
        }
        goto start;
    ...
    default:
         ...
         */
        eapp_print("[ssl3_read_bytes] Before 1734\n");
        SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_READ_BYTES,
    ...
    case SSL3_RT_HANDSHAKE:
         ...
         */
        eapp_print("[ssl3_read_bytes] Before 1746\n");
        SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_READ_BYTES,
                 ERR_R_INTERNAL_ERROR);
        return -1;
    case SSL3_RT_APPLICATION_DATA:
        ...
        if (ossl_statem_app_data_allowed(s)) {
            ...
            eapp_print("[ssl3_read_bytes] Before 1760\n");
            return -1;
            ...
            if (!early_data_count_ok(s, rr->length,
                ...
                /* SSLfatal() already called */
                eapp_print("[ssl3_read_bytes] Before 1775\n");
                return -1;
            }
            SSL3_RECORD_set_read(rr);
            goto start;
        } else {
            eapp_print("[ssl3_read_bytes] Before 1781\n");
            SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE, SSL_F_SSL3_READ_BYTES,
                     SSL_R_UNEXPECTED_RECORD);
            return -1;
        }
