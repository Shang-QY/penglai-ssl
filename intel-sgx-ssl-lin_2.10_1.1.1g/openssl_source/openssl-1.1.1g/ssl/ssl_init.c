...
int OPENSSL_init_ssl(uint64_t opts, const OPENSSL_INIT_SETTINGS * settings)
{
    ...
    int ret = OPENSSL_init_crypto(opts, settings);
    eapp_print("End OPENSSL_init_crypto()\n");
    if (!ret){
        eapp_print("Err OPENSSL_init_crypto()\n");
        return 0;
    } else {
        eapp_print("Correct OPENSSL_init_crypto()\n");
    }
        
    eapp_print("Before ossl_init_ssl_base()\n");
    if (!RUN_ONCE(&ssl_base, ossl_init_ssl_base))
        return 0;
    eapp_print("Before ossl_init_no_load_ssl_strings()\n");
    if ((opts & OPENSSL_INIT_NO_LOAD_SSL_STRINGS)
        && !RUN_ONCE_ALT(&ssl_strings, ossl_init_no_load_ssl_strings,
                         ossl_init_load_ssl_strings))
        return 0;
    eapp_print("Before ossl_init_load_ssl_strings()\n");
