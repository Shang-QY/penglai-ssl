    ...
    do {
        while (s->init_num < SSL3_HM_HEADER_LENGTH) {
            i = s->method->ssl_read_bytes(s, SSL3_RT_HANDSHAKE, &recvd_type,
                                          ...
                                          0, &readbytes);
            if (i <= 0) {
                eapp_print("[tls_get_message_header] Before 1165\n");
            ...
            if (recvd_type == SSL3_RT_CHANGE_CIPHER_SPEC) {
                ...
                if (s->init_num != 0 || readbytes != 1 || p[0] != SSL3_MT_CCS) {
                    eapp_print("[tls_get_message_header] Before 1175\n");
                    SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE,
                ...
                if (s->statem.hand_state == TLS_ST_BEFORE
                    ...
                    eapp_print("[tls_get_message_header] Before 1183\n");
                    /*
                ...
                eapp_print("[tls_get_message_header] Before 1199\n");
                SSLfatal(s, SSL_AD_UNEXPECTED_MESSAGE,
                         SSL_F_TLS_GET_MESSAGE_HEADER,
    ...
    *mt = *p;
    ...
    if (RECORD_LAYER_is_sslv2_record(&s->rlayer)) {
        ...
        if (l > (INT_MAX - SSL3_HM_HEADER_LENGTH)) {
            eapp_print("[tls_get_message_header] Before 1251\n");
