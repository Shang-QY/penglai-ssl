...
static int state_machine(SSL *s, int server)
{
    ...
    eapp_print("[state_machine] Before 310()\n");
    ERR_clear_error();
    clear_sys_error();
    eapp_print("[state_machine] Before 313()\n");
    cb = get_callback(s);
...
#endif
    eapp_print("[state_machine] Before 335()\n");
    ...
    eapp_print("[state_machine] Before 431()\n");
    while (st->state != MSG_FLOW_FINISHED) {
        if (st->state == MSG_FLOW_READING) {
            eapp_print("[state_machine] read");
            ssret = read_state_machine(s);
            eapp_print("[state_machine] read ret %d\n", ssret);
            if (ssret == SUB_STATE_FINISHED) {
                eapp_print("[state_machine] read sub state finish");
                st->state = MSG_FLOW_WRITING;
                init_write_state_machine(s);
            } else {
                eapp_print("[state_machine] read error");
                /* NBIO or error */
                goto end;
            }
        } else if (st->state == MSG_FLOW_WRITING) {
            eapp_print("[state_machine] write\n");
            ssret = write_state_machine(s);
            eapp_print("[state_machine] write ret %d\n", ssret);
            if (ssret == SUB_STATE_FINISHED) {
                eapp_print("[state_machine] write sub state finish");
                st->state = MSG_FLOW_READING;
                init_read_state_machine(s);
            } else if (ssret == SUB_STATE_END_HANDSHAKE) {
                st->state = MSG_FLOW_FINISHED;
            } else {
                eapp_print("[state_machine] write error");
                /* NBIO or error */
                goto end;
            }
        } else {
    ...
    ret = 1;
    eapp_print("[state_machine] Before end()\n");
 end:
    eapp_print("[state_machine] In end() %d\n", ret);
    st->in_handshake--;
    ...
    while (1) {
        ...
        case READ_STATE_HEADER:
            ...
            if (ret == 0) {
                ...
                eapp_print("[read_state_machine] Before 593\n");
             ...
             */
            if (!transition(s, mt)){
                eapp_print("[read_state_machine] Before 609\n");
                return SUB_STATE_ERROR;
            }
            if (s->s3->tmp.message_size > max_message_size(s)) {
                eapp_print("[read_state_machine] Before 613\n");
            ...
            if (!SSL_IS_DTLS(s)
                ...
                eapp_print("[read_state_machine] Before 624\n");
        ...
        case READ_STATE_BODY:
            if (!SSL_IS_DTLS(s)) {
                ...
                if (ret == 0) {
                    eapp_print("[read_state_machine] Before 638\n");
                    /* Could be non-blocking IO */
                    return SUB_STATE_ERROR;
                }
            }
            ...
            if (!PACKET_buf_init(&pkt, s->init_msg, len)) {
                eapp_print("[read_state_machine] Before 646\n");
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_READ_STATE_MACHINE,
                         ERR_R_INTERNAL_ERROR);
                return SUB_STATE_ERROR;
            }
            ...
            case MSG_PROCESS_ERROR:
                eapp_print("[read_state_machine] Before 658\n");
        ...
        case READ_STATE_POST_PROCESS:
            ...
            case WORK_MORE_B:
            case WORK_MORE_C:
                eapp_print("[read_state_machine] Before 687\n");
                return SUB_STATE_ERROR;
        ...
        default:
            eapp_print("[read_state_machine] Before 703\n");
            /* Shouldn't happen */
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_READ_STATE_MACHINE,
                     ERR_R_INTERNAL_ERROR);
            return SUB_STATE_ERROR;
    ...
    while (1) {
        ...
        case WRITE_STATE_TRANSITION:
            ...
            case WRITE_TRAN_ERROR:
                eapp_print("[write_state_machine] Before 822\n");
        ...
        case WRITE_STATE_PRE_WORK:
            ...
            if (!get_construct_message_f(s, &pkt, &confunc, &mt)) {
                /* SSLfatal() already called */
                eapp_print("[write_state_machine] Before 848\n");
                return SUB_STATE_ERROR;
            }
            if (mt == SSL3_MT_DUMMY) {
            ...
            if (!WPACKET_init(&pkt, s->init_buf)
                ...
                WPACKET_cleanup(&pkt);
                eapp_print("[write_state_machine] Before 860\n");
            ...
            if (confunc != NULL && !confunc(s, &pkt)) {
                WPACKET_cleanup(&pkt);
                eapp_print("[write_state_machine] Before 867\n");
                check_fatal(s, SSL_F_WRITE_STATE_MACHINE);
                return SUB_STATE_ERROR;
            }
            if (!ssl_close_construct_packet(s, &pkt, mt)
                    || !WPACKET_finish(&pkt)) {
                WPACKET_cleanup(&pkt);
                eapp_print("[write_state_machine] Before 874\n");
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_WRITE_STATE_MACHINE,
                         ERR_R_INTERNAL_ERROR);
                return SUB_STATE_ERROR;
            }
        ...
        case WRITE_STATE_SEND:
            ...
            eapp_print("[write_state_machine] Before statem_do_write\n");
            ret = statem_do_write(s);
            if (ret <= 0) {
                eapp_print("[write_state_machine] Before 888\n");
                return SUB_STATE_ERROR;
            }
            st->write_state = WRITE_STATE_POST_WORK;
            st->write_state_work = WORK_MORE_A;
        ...
        case WRITE_STATE_POST_WORK:
            ...
            case WORK_MORE_B:
            case WORK_MORE_C:
                eapp_print("[write_state_machine] Before 903\n");
                return SUB_STATE_ERROR;
        ...
        default:
            eapp_print("[write_state_machine] Before 916\n");
            SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_F_WRITE_STATE_MACHINE,
                     ERR_R_INTERNAL_ERROR);
            return SUB_STATE_ERROR;
        }
